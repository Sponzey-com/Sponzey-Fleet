...
sponzey agent enroll --url https://fleet.example.com --token <enrollment-token>
sponzey agent start
```
...
sponzey controller start --host 0.0.0.0 --port 7700
```
...
npm install -g @sponzey/fleet
sponzey controller init \
  --db postgres://fleet:secret@127.0.0.1:5432/fleet \
  --external-url https://fleet.example.com

sponzey controller install-service --user sponzey
sponzey controller start-service
```
...
```bash
npm install -g @sponzey/fleet
sponzey agent enroll --url https://fleet.example.com --token <token>
sponzey agent start
```
...
| 명령                           | 의미                                      | 실제 내부 동작                                                         |
| ---------------------------- | --------------------------------------- | ---------------------------------------------------------------- |
| `sponzey agent install`      | agent 설치/등록/서비스화를 한 번에 처리               | `agent enroll` + `agent install-service` + `agent start-service` |
...
- `sponzey agent enroll --url ... --token ...` 성공
...
```bash
npm install -g @sponzey/fleet
sudo sponzey agent enroll --url https://fleet.example.com --token <token> --labels role=web
sudo sponzey agent install-service
```
